...
//
// Styling: themed CSS variables (correct in both light and dark) — Tailwind's
// `dark:` variant is bound to `.dark`, which this app never sets.
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium ${
        isHold ? "border border-amber-500/40 bg-amber-500/10 text-amber-500" : ""
      }`}
      style={
        isHold
          ? undefined
          : { border: "1px solid var(--divider)", color: "var(--panel-text-muted)" }
      }
    >
  ...
  if (action === "evidence.access") {
    ...
    return (
      <span className="inline-flex items-center gap-1">
        ...
        {details?.source ? <span className="opacity-70">· {String(details.source)}</span> : null}
      </span>
    );
  }
...
export default function EvidenceAccessPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-8" style={{ color: "var(--panel-text)" }}>
      <header className="mb-6">
        <h1 className="flex items-center gap-2 text-2xl font-semibold">
          <ScrollText className="h-6 w-6" style={{ color: "var(--section-heading)" }} />
          Evidence Access Log
        </h1>
      ...
      {rows && rows.length === 0 && (
        <div
          ...
          style={{ border: "1px solid var(--divider)", color: "var(--panel-text-muted)" }}
        >
          No evidence has been accessed yet. Open an evidence link on the Compliance
          page and it will appear here.
        </div>
      )}

      {rows && rows.length > 0 && (
        <div className="overflow-x-auto rounded-xl" style={{ border: "1px solid var(--divider)" }}>
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.seq} style={{ borderTop: "1px solid var(--row-border)" }}>
                  <td
                  ...
                  <td className="px-4 py-2.5">
                    {r.target ? (
                      <EvidenceLink
                        uri={r.target}
                        className="font-mono text-[11px] text-emerald-500 hover:underline"
                      >
                        {r.target.slice(0, 16)}…
                      </EvidenceLink>
                    ) : (
                  ...
                  <td className="px-4 py-2.5 text-xs" style={{ color: "var(--panel-text-muted)" }}>
