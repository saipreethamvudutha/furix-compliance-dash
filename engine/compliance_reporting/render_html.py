...
def _framework_card(fw: Mapping[str, Any]) -> str:
    ...
    pct_label = "n/a" if pct is None else f"{pct:g}%"
    angle = 0 if pct is None else pct * 3.6
    donut = (
        f'<div class="donut" style="background:conic-gradient(var(--ok) {angle}deg,'
        f'var(--bad-bg) {angle}deg 360deg)"><span>{_esc(pct_label)}</span></div>'
    )
    return f"""
<div class="card">
  ...
  <div class="ring">{donut}
    <div class="legend">
      <div><b>{fw["requirements_compliant"]}</b> compliant</div>
...
def _controls_table(report: Mapping[str, Any]) -> str:
    rows = []
    order = {"at_risk": 0, "compliant": 1, "not_monitored": 2}
    for c in sorted(report["controls"], key=lambda c: (order[c["status"]], int(c["control_id"].split()[-1]))):
...
def _test_details(report: Mapping[str, Any]) -> str:
    blocks = []
    order = {"fail": 0, "pass": 1, "no_data": 2}
    for t in sorted(report["tests"], key=lambda t: (order[t["status"]], t["test_id"])):
        evidence_rows = "".join(
...
<h2>Tests ({s["tests_failed"]} failed / {s["tests_passed"]} passed of {s["tests_total"]})</h2>
